...
#include <glm/gtc/matrix_access.hpp> // 包含 glm::row 函数
#include <vector>
#include <random>
#include <functional>
...
std::vector<float> quadVertices{
	...
	-1.0, 1.0, 0.0,  0.0, 1.0,  // 左上,0
    -1.0, -1.0, 0.0, 0.0, 0.0,  // 左下,1
    1.0, 1.0, 0.0,   1.0, 1.0,  // 右上,2
    1.0, -1.0, 0.0,  1.0, 0.0   // 右下,3
};
...
glm::vec3 normal(0.0, 0.0, 1.0);// normal vector, 平面, 仅一个法线
...
float heightScale = 0.05; // 纹理偏移比例系数
void key_callback(GLFWwindow *window, int key, int scancode, int action, int mods) {
	switch (key) {
		...
		case GLFW_KEY_K : { //增加 heightScale
			heightScale += 0.005;
			if(heightScale > 1.00005) heightScale = 1.f;
			break;
		}
		case GLFW_KEY_J : {//降低 heightScale
			heightScale -= 0.005;
			if(heightScale < 0.00001) heightScale = 0.f;
			break;
		}
		default:{
		}
