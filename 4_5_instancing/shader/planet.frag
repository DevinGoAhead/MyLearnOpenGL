...
void main() {
	...
	float spec = pow(max(dot(point2Camera, reflectlight), 0.f), uMaterial.shininess);
	vec3 resultSpec = texture(uMaterial.textureSpecular0, vTexCoord).rgb * uLight.specular * diff * spec;
